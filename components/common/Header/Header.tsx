  ...
  return (
    ...
    >
      <nav className="relative items-center py-5 px-4 xl:px-4 mx-auto  xl:flex xl:gap-4 xl:justify-between">
        ...
        >
          <div className="w-full flex flex-col xl:flex-row justify-center items-start xl:justify-between xl:items-center gap-5 xl:gap-2.5 py-5 xl:py-0 px-5 lg:px-10 xl:px-0">
            {navData.map(({ label, link }, idx) => (
              <Link
                key={idx}
                href={link}
                className="text-white hover:text-black text-base font-normal text-center transition-all ease-in-out delay-75"
              >
                {label}
              </Link>
            ))}
